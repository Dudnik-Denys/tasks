class RadiusVector2D:
    ...
    @x.setter
    def x(self, x):
        self.__x = x if type(x) in (int, float) and \
            RadiusVector2D.MIN_COORD <= x <= RadiusVector2D.MAX_COORD else self.__x
    ...
    @y.setter
    def y(self, y):
        self.__y = y if type(y) in (int, float) and \
            RadiusVector2D.MIN_COORD <= y <= RadiusVector2D.MAX_COORD else self.__y
